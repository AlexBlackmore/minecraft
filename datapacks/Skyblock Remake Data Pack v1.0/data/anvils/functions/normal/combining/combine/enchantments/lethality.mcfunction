...
execute store result score @s Anvil_Item1 run data get storage blue:anvil Items[{Slot:20b}].tag.Base.Enchantments[{lethality:{identifier:1b}}].lethality.level
execute store result score @s Anvil_Item2 run data get storage blue:anvil Items[{Slot:24b}].tag.Base.Enchantments[{lethality:{identifier:1b}}].lethality.level
...
execute unless data storage blue:anvil ResultItem.tag.Base.Enchantments[].lethality run data modify storage blue:anvil ResultItem.tag.Base.Enchantments append value {lethality:{identifier:1b,level:1}}
#--------SET LEVEL TO DECIDED VALUE--------#
execute store result storage blue:anvil ResultItem.tag.Base.Enchantments[{lethality:{identifier:1b}}].lethality.level int 1 run scoreboard players get @s Anvil_Item1
#---------------ADD XP COST----------------#
