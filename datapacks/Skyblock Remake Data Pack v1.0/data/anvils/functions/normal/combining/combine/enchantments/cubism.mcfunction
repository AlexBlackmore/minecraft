...
execute store result score @s Anvil_Item1 run data get storage blue:anvil Items[{Slot:20b}].tag.Base.Enchantments[{cubism:{identifier:1b}}].cubism.level
execute store result score @s Anvil_Item2 run data get storage blue:anvil Items[{Slot:24b}].tag.Base.Enchantments[{cubism:{identifier:1b}}].cubism.level
...
execute unless data storage blue:anvil ResultItem.tag.Base.Enchantments[].cubism run data modify storage blue:anvil ResultItem.tag.Base.Enchantments append value {cubism:{identifier:1b,level:1}}
#--------SET LEVEL TO DECIDED VALUE--------#
execute store result storage blue:anvil ResultItem.tag.Base.Enchantments[{cubism:{identifier:1b}}].cubism.level int 1 run scoreboard players get @s Anvil_Item1
#---------------ADD XP COST----------------#
