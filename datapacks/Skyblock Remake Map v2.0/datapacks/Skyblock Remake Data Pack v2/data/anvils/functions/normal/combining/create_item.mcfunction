...
execute if data storage blue:anvil Items[{Slot:24b}].tag.Base.canApplyScrolls if data storage blue:anvil {ResultItem:{tag:{Base:{CanHaveScrolls:1b}}}} run function anvils:normal/combining/item_type/wither_scrolls
execute if data storage blue:anvil Items[{Slot:24b}].tag.Base.PotatoBoost if data storage blue:anvil ResultItem run function anvils:normal/combining/item_type/potato_book

scoreboard players operation @s Anvil_Level /= c10 Constant


#------------ADD ENCHANT GLINT-------------#
execute if entity @s[tag=!ScrollSuccess,tag=!PotatoBookSuccess] if data storage blue:anvil ResultItem.tag.Base.Enchantments[0] run function anvils:normal/combining/create_item_succeed
execute if entity @s[tag=!ScrollSuccess,tag=!PotatoBookSuccess] unless data storage blue:anvil ResultItem.tag.Base.Enchantments[0] run function anvils:normal/combining/create_item_failed
#--------------WITHER SCROLLS--------------#
execute if entity @s[tag=ScrollSuccess,tag=!PotatoBookSuccess] run function anvils:normal/combining/create_item_succeed
#-------------HOT POTATO BOOKS-------------#
execute if entity @s[tag=!ScrollSuccess,tag=PotatoBookSuccess] run function anvils:normal/combining/create_item_succeed
#------------------RESETS------------------#
tag @s remove ScrollSuccess
tag @s remove PotatoBookSuccess
#------------------------------------------#
