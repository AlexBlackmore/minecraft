...
execute if score $PlayerCoins Temp < $QuestCost Temp run tellraw @p {"text":"You don't have enough Coins to afford the quest ! :( ","color":"red"}
execute if score $PlayerCoins Temp < $QuestCost Temp run playsound minecraft:entity.villager.no master @p ~ ~ ~ 1 1 1

execute if score $PlayerCoins Temp >= $QuestCost Temp if score SlayerChosen Temp matches 1 run scoreboard players set @s M_NewPage 242
execute if score $PlayerCoins Temp >= $QuestCost Temp if score SlayerChosen Temp matches 2 run scoreboard players set @s M_NewPage 244
execute if score $PlayerCoins Temp >= $QuestCost Temp if score SlayerChosen Temp matches 3 run scoreboard players set @s M_NewPage 246