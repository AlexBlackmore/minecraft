execute store result score @s dummy run data get entity @s[nbt={Inventory:[{Slot:-106b,tag:{Name:"Enchanted Emerald Rare"}}]}] Inventory[{Slot:-106b}].Count
tag @s[scores={dummy=2..}] add success
tellraw @s[tag=!success] ["",{"text":"You do not have enough ","color":"red"},{"text":"Enchanted Emeralds","color":"aqua"},{"text":" in your offhand to start this quest!","color":"red"}]
...
advancement grant @s[tag=success] only skyblock:quests/slayer/zombie/tier4
