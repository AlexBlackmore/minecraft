execute if data entity @s SelectedItem.tag.Enchantments[{id:"minecraft:impaling"}] run tag @s add fail
execute if data entity @s SelectedItem.tag.Enchantments[{id:"minecraft:impaling"}] run tellraw @s {"text":"This item already has a higher level of that enchantment!","color":"red"}
...
item modify entity @s[tag=success] weapon.mainhand skyblock:enchantments/increase_impaling
tellraw @s[tag=success] ["",{"text":"You enchanted your ","color":"white"},{"nbt":"SelectedItem.tag.display.Name","interpret":true,"entity":"@s"},{"text":" with "},{"text":"Impaling I","color":"gray"},{"text":"!","color":"white"}]
