execute if data entity @s SelectedItem.tag.Enchantments[{id:"minecraft:looting"}] run tag @s add fail
execute if data entity @s SelectedItem.tag.Enchantments[{id:"minecraft:looting"}] run tellraw @s {"text":"This item already has a higher level of that enchantment!","color":"red"}
...
item modify entity @s[tag=success] weapon.mainhand skyblock:enchantments/increase_looting
tellraw @s[tag=success] ["",{"text":"You enchanted your ","color":"white"},{"nbt":"SelectedItem.tag.display.Name","interpret":true,"entity":"@s"},{"text":" with "},{"text":"Looting I","color":"gray"},{"text":"!","color":"white"}]
