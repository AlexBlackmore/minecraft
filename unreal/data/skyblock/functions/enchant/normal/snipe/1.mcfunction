execute if data entity @s SelectedItem.tag.CustomEnchantments[{id:"skyblock:snipe"}] run tag @s add fail
execute if data entity @s SelectedItem.tag.CustomEnchantments[{id:"skyblock:snipe"}] run tellraw @s {"text":"This item already has a higher level of that enchantment!","color":"red"}
...
data modify entity @e[tag=anvil,limit=1,sort=nearest] HandItems[0].tag.CustomEnchantments insert 0 value {id:"skyblock:snipe",lvl:1}
...
tellraw @s[tag=success] ["",{"text":"You enchanted your ","color":"white"},{"nbt":"SelectedItem.tag.display.Name","interpret":true,"entity":"@s"},{"text":" with "},{"text":"Snipe I","color":"blue"},{"text":"!","color":"white"}]
