...
tellraw @s[scores={dummy=6..}] {"text":"This item already has a higher level of that enchantment!","color":"red"}
tellraw @s[level=0..59] {"text":"You don't have enough Experience Levels to apply that enchantment!","color":"red"}
tag @s[scores={dummy=6..}] add fail
tag @s[level=0..59] add fail
...
execute as @e[tag=anvil,limit=1,sort=nearest] unless data entity @s HandItems[0].tag.CustomEnchantments[{id:"skyblock:growth"}] run data modify entity @s HandItems[0].tag.CustomEnchantments insert 0 value {id:"skyblock:growth",lvl:6}
execute as @e[tag=anvil,limit=1,sort=nearest] if data entity @s HandItems[0].tag.CustomEnchantments[{id:"skyblock:growth"}] run data modify entity @s HandItems[0].tag.CustomEnchantments[{id:"skyblock:growth"}].lvl set value 6
...
tellraw @s[tag=success] ["",{"text":"You enchanted your ","color":"white"},{"nbt":"SelectedItem.tag.display.Name","interpret":true,"entity":"@s"},{"text":" with "},{"text":"Growth VI","color":"blue"},{"text":"!","color":"white"}]
xp add @s[tag=success] -60 levels
