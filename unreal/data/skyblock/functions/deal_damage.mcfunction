...
execute as @e[tag=target,scores={absorption=1..}] at @s run tellraw @p ["",{"entity":"@s","nbt":"CustomName","interpret":true},{"text":" "},{"score":{"name":"@s","objective":"health"},"color":"red"},{"text":"❤","color":"red"}]
